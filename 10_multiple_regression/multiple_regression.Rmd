...
4.  Replot the scatterplot, but this time use `geom_jitter` as your layer. What was
...
after you've accounted for the color of the professor's picture, you can add the 
picture color term into the model.
...
8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `pic_color` to the model changed the parameter estimate for `bty_avg`?

Note that the estimate for `pic_color` is now called `pic_colorcolor`. You'll see this 
name change whenever you introduce a categorical variable. The reason is that R 
recodes `pic_color` from having the values of `color` and `black&white` to being an 
indicator variable called `pic_colorcolor` that takes a value of $0$ for 
black and white pictures and a value of $1$ for those in color. (Such variables 
are often referred to as "dummy" variables.)

As a result, for those with black and white pictures, the parameter estimate is 
multiplied by zero, leaving the intercept and slope form familiar from simple regression.
...
The decision to call the indicator variable `pic_color_color` instead of 
`pic_color_back&white` has no deeper meaning. R simply codes the category that 
...
10. Create a new model called `m_bty_rank` with `pic_color` removed and `rank` 
...
ethnicity, language of the university where they got their degree, age, 
