...
and many others in the future, we will use a suite of R packages that are known as 
the 'tidyverse', as well as a package specifically written to accompany the OpenIntro labs (`oilabs`). 
...
This command will only show the number of boys baptized each year. The select function pulls
a variable from the dataset it comes from. 

1. What command would you use to extract just the counts of girls baptized? Try
    it!
    
Notice that R has added numbers along the left side of the printout to indicate 
locations within the vector. For example, 5218 follows 1, indicating that 5218 
is the first entry in the vector. And if 43 starts a line, then that would 
mean the first number on that line would represent
...
that of girls, R will compute all sums simultaneously. To do this, we can use
the `mutate()` function to create a new variable, `total`.
...
<div id="boxedtext">
**Where is the new variable? ** If you click on the name of the dataset again to update
it in the data viewer, you will notice that it still does not have a column labeled 
`total`. This is because we haven't overwritten the old version of `arbuthnot`, just
tried making a new one to make sure it looked right. 
</div>
...
<div id="boxedtext">
**The assignment operator** This code does much the same thing as the chunk above. 
However, it "assigns" the resulting dataset to the object called `arbuthnot`.
This means we overwrite the old `arbuthnot` dataset with the new one
...
output of some code and saving it into an object in your workspace. In 
...
Finally, we can use the `arrange` function to sort the data according to some variable. 
In this case, we might be interested in the years in the `arbuthnot` data with the highest
totals. If we `arrange` in **desc**ending order, we can see that information. 
...
7.  In what year did we see the most total number of births in the U.S.? *Hint:*
    ...
    dataset in descending order based on the total column. 
