...
<div id="boxedtext">
**Setting a seed:** You will take some random samples and build sampling distributions
...
population using the `rep_sample_n` function from `infer`. Although `rep_sample_n`
has more arguments that you may use in the future, you just need to include the
size of the sample since we are just taking one sample without replacement.
...
command consistent with the `tidyverse` framework, thus making it consistent for
coding purposes. You will use these commands to generate a confidence interval:

Command | Purpose
------- | -------
`specify` | Identify your variable of interest using the `formula` argument
`generate` | The amount of repetitions you want to do
`calculate` | The type of statistic you are looking for
...
Feel free to research the rest of the arguments for these functions, since this
framework will be used to calculate any confidence intervals, and do any inference
problem the rest of the semester.
...
In this case, you have the rare luxury of knowing the true population mean since we 
have data on the entire population. Let's calculate this value so that
you can determine if your confidence intervals actually capture it. You can store
it in a data frame called `params` (short for population parameters), and name it `mu`.
...
-   Specify the variable you seek for the confidence interval
...
of the confidence intervals based on these samples.
...
We now have all the information we need to create the plot. Note that the 
`geom_errorbar()` function only understands `y` values, and thus we have used the
`coord_flip()` function to flip the coordinates of the entire plot back to the 
more familiar vertical orientation. 
