...
This week you'll be working with measurements of body dimensions.  This data set 
...
You'll see that for every observation there are 25 measurements, many of which are
either diameters or girths.  You can learn more about the variables by bringing 
up their help page.
...
You'll be focusing on just three columns to get started: weight in kg (`wgt`), 
...
You'll be focusing on women's heights, so let's store them as a separate object 
...
After initializing a blank plot with `geom_blank()`, the `ggplot2` package
...
some errant points towards the tails.  You're left with the same problem that we
